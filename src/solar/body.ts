...
export class Body {
    ...
    _E(): Angle {
        let E0 =
            this.M.asRad() +
            this.e *
                Math.sin(this.M.asRad()) *
                (1 + this.e * Math.cos(this.M.asRad()));
        for (let i = 0; i < 25; i++) {
            let E1 =
                E0 -
                (E0 - this.e * Math.sin(E0) - this.M.asRad()) /
                    (1 - this.e * Math.cos(E0));
    ...
    _dist(xyz: RectangularCoordinate): number {
        return Math.sqrt(
            Math.pow(xyz.x.asRad(), 2) + Math.pow(xyz.y.asRad(), 2)
        );
    }
    ...
    asEclip(): RectangularCoordinate {
        ...
        return new RectangularCoordinate(
            new Angle(
                r *
                    (Math.cos(this.N.asRad()) *
                        Math.cos(v.asRad() + this.w.asRad()) -
                        Math.sin(this.N.asRad()) *
                            Math.sin(v.asRad() + this.w.asRad()) *
                            Math.cos(this.i.asRad()))
            ),
            new Angle(
                r *
                    (Math.sin(this.N.asRad()) *
